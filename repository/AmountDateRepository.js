...
class AmountDateRepository {
  ...
  async setProfitAmount(profitAmount, date) {
    const selectQuery = `SELECT amount FROM profit_amount_date WHERE date = ?;`;
    this.db.transaction(tx => {
      tx.executeSql(selectQuery, [date], (tx, results) => {
        if (results.rows.length > 0) {
          ...
          const updateQuery = `UPDATE profit_amount_date SET amount = ? WHERE date = ?;`;
          ...
          error => {
              ...
              const insertQuery = `INSERT INTO profit_amount_date (date, amount) VALUES (?, ?);`;
  ...
  async setSellAmount(sellAmount, date) {
    const selectQuery = `SELECT amount FROM sell_amount_date WHERE date = ?;`;
    this.db.transaction(tx => {
      tx.executeSql(selectQuery, [date], (tx, results) => {
        if (results.rows.length > 0) {
          ...
          const updateQuery = `UPDATE sell_amount_date SET amount = ? WHERE date = ?;`;
          ...
          // If no record with the given date exists, insert a new record
          const insertQuery = `INSERT INTO sell_amount_date (date, amount) VALUES (?, ?);`;
